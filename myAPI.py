...
def send_message():
        ...
        if msg == None or chatroom_id == None or chatroom_id == '' or not chatroom_id.isdigit() or name == None or user_id == None :
                ...
                str_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()+8*3600))

                params = (chatroom_id,user_id,name,msg,str_time)
        ...
        #               conn.send(json.dumps({'data':'new message'}))
                ...
                ws_msg = { "chatroom_id": chatroom_id, "user_id": user_id, "name": name, "message": msg, "message_time": str_time}
